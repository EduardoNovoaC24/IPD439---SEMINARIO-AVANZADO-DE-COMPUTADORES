...
void run_memcpy_test(uint16_t len);
...
void run_memcpy_test(uint16_t len)
{
  ...
  memcpy(dst_sram, src_flash, len);
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET);

  memcpy_ok = verify_copy(src_flash, dst_sram, len);
}
/* USER CODE END 0 */
